...
exports.TOTAL_TOLERANCE = exports.AMOUNT_TOLERANCE = void 0;
exports.getVisibleSheetNames = getVisibleSheetNames;
exports.buildExcelAudit = buildExcelAudit;
exports.attachExcelAudit = attachExcelAudit;
const XLSX = __importStar(require("xlsx"));
const dpr_boq_validation_util_1 = require("./dpr-boq-validation.util");
exports.AMOUNT_TOLERANCE = 0.5;
exports.TOTAL_TOLERANCE = 1.0;
...
function totalRowTolerance(amount) {
    ...
    return Math.max(0.05, exports.TOTAL_TOLERANCE, base * 0.00001);
}
function cellRef(col, row) {
    return XLSX.utils.encode_cell({ r: row, c: col });
}
function columnLabel(col, headerMap, field) {
    if (field && headerMap?.[field] !== undefined) {
        return field.charAt(0).toUpperCase() + field.slice(1);
    }
    return XLSX.utils.encode_col(col);
}
function refColumn(cellRefStr) {
    if (!cellRefStr)
        return '';
    try {
...
function evalCellRef(sheet, ref) {
    const addr = ref.replace(/\$/g, '').toUpperCase();
    return parseNumber(sheet[addr]?.v);
}
function evalArithmeticChain(sheet, expr) {
...
function evaluateFormula(sheet, formula) {
    ...
    if (!expr)
        return null;
    if (/^#REF!|#VALUE!|#DIV\/0!|#N\/A|#NAME\?|#NULL!/i.test(expr))
        return null;
    ...
    if (sumMatch) {
        ...
        for (const part of parts)
            total = round2(total + sumRange(sheet, part));
        return total;
    }
    ...
    if (pctMatch)
        return round2(evalCellRef(sheet, pctMatch[1]) * evalCellRef(sheet, pctMatch[2]) / 100);
    ...
    if (mulMatch)
        return round2(evalCellRef(sheet, mulMatch[1]) * evalCellRef(sheet, mulMatch[2]));
...
function isTotalLabel(text) {
    return (0, dpr_boq_validation_util_1.isSubTotalLabel)(text) || (0, dpr_boq_validation_util_1.isSectionTotalLabel)(text) || (0, dpr_boq_validation_util_1.isTotalCostLabel)(text) || (0, dpr_boq_validation_util_1.isGrandTotalLabel)(text);
}
function auditFormulasOnSheet(sheet, sheetName, pageNo) {
    const errors = [];
    const rows = [];
    let verified = 0;
    let unverified = 0;
    for (const addr of Object.keys(sheet)) {
        ...
        verified += 1;
        const match = Math.abs(displayed - computed) <= exports.AMOUNT_TOLERANCE;
        rows.push({
            sheetName, pageNo, cellRef: addr, rowNo, formula: cell.f,
...
function auditDataQuality(page, rows, headerMap, headerRowIdx) {
    const errors = [];
    ...
    for (let i = headerRowIdx + 1; i < rows.length; i += 1) {
        ...
        if (headerMap.qty !== undefined && headerMap.rate !== undefined && headerMap.amount !== undefined) {
            ...
            if (qty > 0 && rate > 0 && amount > 0) {
                ...
                if (Math.abs(amount - expected) > exports.AMOUNT_TOLERANCE) {
...
function auditOverwrittenFormulas(sheet, page, headerMap, headerRowIdx, rows) {
    const errors = [];
    if (headerMap.qty === undefined || headerMap.rate === undefined || headerMap.amount === undefined)
        return errors;
    for (let i = headerRowIdx + 1; i < rows.length; i += 1) {
        ...
        const rowNo = i + 1;
        const expected = round2(qty * rate);
        if (cell && !cell.f && Math.abs(amount - expected) <= exports.AMOUNT_TOLERANCE) {
            errors.push({
...
function issueToErrorType(checkType) {
    ...
    if (checkType === 'rate')
        return 'Invalid rate';
    if (checkType === 'qty_rate_ujn' || checkType === 'qty_rate_amount')
        return 'Qty × Rate ≠ Total Amount';
    if (checkType === 'component_sum')
        return 'Component sum ≠ Total Amount';
    if (checkType === 'cross_check')
        return 'Qty×Rate ≠ component sum';
    return 'Data warning';
}
function cellRefForIssue(page, line, issue) {
    ...
    if (field && map[field] !== undefined)
        return cellRef(map[field], rowIdx);
    if (issue.column === 'Unit' && map.unit !== undefined)
        return cellRef(map.unit, rowIdx);
    return line.sheetRow ? amountColRef(page, line.sheetRow) : '';
}
function lineToAuditErrors(page, line) {
    const failedIssues = (line.issues ?? []).filter((i) => i.status !== 'pass');
    if (failedIssues.length > 0) {
        return failedIssues.map((issue) => {
            const severity = issue.status === 'fail' ? 'major' : 'minor';
    ...
    if (line.status === 'pass')
        return [];
    const severity = line.status === 'fail' ? 'major' : 'minor';
...
function totalCheckToErrors(page, check) {
    const errors = [];
...
function crossCheckToError(check, page) {
...
function buildSheetsSummary(pages, visible, blockingErrors) {
    const errorsBySheet = new Map();
    ...
    return visible.map((sheetName) => {
        ...
        if (!page?.isCalculationSheet) {
            return { sheetName, status: 'skipped' };
        }
...
function buildExcelAudit(workbook, visible, hidden, pages, crossChecks, baseReport) {
    const errors = [];
    const formulaAudits = [];
    ...
    for (const page of pages) {
        ...
        for (const line of page.lines) {
            if (line.status !== 'pass')
                errors.push(...lineToAuditErrors(page, line));
        }
        for (const check of page.totalChecks) {
            if (!check.match)
                errors.push(...totalCheckToErrors(page, check));
        }
...
function attachExcelAudit(workbook, visible, hidden, report) {
    try {
        ...
        const finalStatus = hasBlockingAuditErrors
            ? 'failed'
            : report.status;
