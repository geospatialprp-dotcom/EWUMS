"use strict";
/**

 * Inline regression tests for Step 6/7 BOQ section totals.

 * Run: npm run test:boq-section-totals

 */
Object.defineProperty(exports, "__esModule", { value: true });
const dpr_boq_validation_util_1 = require("./dpr-boq-validation.util");
const THARALI_HEADER = {
    serial: 0,
    description: 1,
    unit: 2,
    qty: 3,
    rate: 4,
    dsr: 5,
    ujn: 6,
    sor_pwd: 7,
    nsi: 8,
    total_amount: 9,
};
function assert(condition, message) {
    if (!condition)
        throw new Error(message);
}
/** Reproduces "so & Tra" row 11 — Step 7 Total Cost must equal Excel, not 2×. */
function testSoTraStep7TotalCostRow11() {
    const DSR = 2704964;
    const UJN = 1296;
    const SOR = 393668.98;
    const TOTAL = 3099928.98;
    const rows = [
        ['S.No', 'Description', 'Unit', 'Qty', 'Rate', 'DSR', 'UJN', 'SOR(PWD)', 'NSI', 'Total Amount'],
        [1, 'Earthwork item', 'cum', 100, 27049.64, DSR, UJN, SOR, 0, TOTAL],
        [2, 'Sub Total', '', '', '', DSR, UJN, SOR, 0, TOTAL],
        // Description col blank; "Total Cost" only in joined text — auditVerticalAmountColumn used to double-count this row.
        [11, '', 'Total Cost', '', '', DSR, UJN, SOR, 0, TOTAL],
    ];
    const checks = (0, dpr_boq_validation_util_1.validateTotalRows)(rows, THARALI_HEADER, 1, TOTAL);
    const totalCost = checks.find((c) => c.checkStep === 7 && c.rowNo === 4);
    assert(!!totalCost, 'Expected Step 7 Total Cost check at row 4');
    assert(totalCost.match === true, `Total Cost should pass; got: ${totalCost.message}`);
    const dsrCol = totalCost.columnChecks?.find((c) => c.columnLabel === 'DSR');
    assert(dsrCol?.computedAmount === DSR, `DSR calculated must be ${DSR}, got ${dsrCol?.computedAmount}`);
    assert(dsrCol?.computedAmount !== DSR * 2, `DSR must not be double-counted (${DSR * 2})`);
    const ujnCol = totalCost.columnChecks?.find((c) => c.columnLabel === 'UJN');
    assert(ujnCol?.computedAmount === UJN, `UJN calculated must be ${UJN}, got ${ujnCol?.computedAmount}`);
    const totalCol = totalCost.columnChecks?.find((c) => c.columnLabel === 'Total Amount');
    assert(Math.abs((totalCol?.computedAmount ?? 0) - TOTAL) <= 1, `Total Amount calculated must be ${TOTAL}, got ${totalCol?.computedAmount}`);
}
/** cwra-style amount-only rows in Amount column must contribute to Step 6 UJN subtotal. */
function testCwraAmountOnlyRowsStep6() {
    const UJN = 951892;
    const header = {
        ...THARALI_HEADER,
        amount: 10,
    };
    const rows = [
        ['S.No', 'Description', 'Unit', 'Qty', 'Rate', 'DSR', 'UJN', 'SOR(PWD)', 'NSI', 'Total Amount', 'Amount'],
        [1, 'Lump sum item', 'LS', 0, 0, 0, 0, 0, 0, 0, UJN],
        [2, 'Sub Total', '', '', '', 0, UJN, 0, 0, UJN, ''],
    ];
    const checks = (0, dpr_boq_validation_util_1.validateTotalRows)(rows, header, 1, UJN);
    const subTotal = checks.find((c) => c.checkStep === 6);
    assert(!!subTotal, 'Expected Step 6 Sub Total check');
    assert(subTotal.match === true, `Sub Total should pass; got: ${subTotal.message}`);
    const ujnCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'UJN');
    assert(ujnCol?.computedAmount === UJN, `UJN calculated must be ${UJN}, got ${ujnCol?.computedAmount}`);
}
/** cwra production — row 8 item + row 9 description-only + row 10 SOR items → row 11 Sub Total. */
function testCwraRows8_10_11Step6() {
    const UJN = 951892;
    const NSI = 13400;
    const SOR1 = 57917.48;
    const SOR2 = 38611.65;
    const SOR = 96529.13;
    const TOTAL = 965292;
    const header = { ...THARALI_HEADER };
    const rows = [
        ['S.No', 'Description', 'Unit', 'Qty', 'Rate', 'DSR', 'UJN', 'SOR(PWD)', 'NSI', 'Total Amount'],
        [8, 'Main lump sum item', 'LS', 0, 0, 0, UJN, 0, NSI, UJN + NSI],
        [9, 'Protection works for substructure and foundation', '', '', '', 0, 0, 0, 0, 0],
        [10, 'SOR rated item A', 'Nos', 1, SOR1, 0, 0, SOR1, 0, SOR1],
        [10, 'SOR rated item B', 'Nos', 1, SOR2, 0, 0, SOR2, 0, SOR2],
        [11, 'Sub Total', '', '', '', 0, UJN, SOR, NSI, TOTAL],
    ];
    const checks = (0, dpr_boq_validation_util_1.validateTotalRows)(rows, header, 1, TOTAL);
    const subTotal = checks.find((c) => c.checkStep === 6 && c.rowNo === 6);
    assert(!!subTotal, 'Expected Step 6 Sub Total check at row 11 (index 6)');
    assert(subTotal.match === true, `Sub Total should pass; got: ${subTotal.message}`);
    const ujnCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'UJN');
    assert(ujnCol?.computedAmount === UJN, `UJN calculated must be ${UJN}, got ${ujnCol?.computedAmount}`);
    const sorCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'SOR(PWD)');
    assert(Math.abs((sorCol?.computedAmount ?? 0) - SOR) <= 0.05, `SOR(PWD) calculated must be ${SOR}, got ${sorCol?.computedAmount}`);
    const nsiCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'NSI');
    assert(nsiCol?.computedAmount === NSI, `NSI calculated must be ${NSI}, got ${nsiCol?.computedAmount}`);
    const totalCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'Total Amount');
    assert(totalCol?.computedAmount === TOTAL, `Total Amount calculated must be ${TOTAL}, got ${totalCol?.computedAmount}`);
}
/** cwra row 11 — lump sums in unmapped Amt column + NSI; prior items rows 1–7, subtotal at row 11. */
function testCwraUnmappedAmtColumnStep6() {
    const UJN = 951892;
    const NSI = 13400;
    const TOTAL = UJN + NSI;
    const header = { ...THARALI_HEADER };
    const rows = [
        ['S.No', 'Description', 'Unit', 'Qty', 'Rate', 'DSR', 'UJN', 'SOR(PWD)', 'NSI', 'Total Amount', 'Amt'],
        [1, 'Lump sum item A', 'LS', 0, 0, 0, 0, 0, 0, 0, 500000],
        [2, 'Lump sum item B', 'LS', 0, 0, 0, 0, 0, 0, 0, 451892],
        [3, 'NSI rated item', 'LS', 0, 0, 0, 0, 0, NSI, NSI, 0],
        [9, 'Protection works for substructure', '', '', '', 0, 0, 0, 0, 0, 0],
        [11, 'Sub Total', '', '', '', 0, UJN, 0, NSI, TOTAL, ''],
    ];
    const checks = (0, dpr_boq_validation_util_1.validateTotalRows)(rows, header, 1, TOTAL);
    const subTotal = checks.find((c) => c.checkStep === 6 && c.rowNo === 6);
    assert(!!subTotal, 'Expected Step 6 Sub Total check at row 11');
    assert(subTotal.match === true, `Sub Total should pass; got: ${subTotal.message}`);
    const ujnCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'UJN');
    assert(ujnCol?.computedAmount === UJN, `UJN calculated must be ${UJN}, got ${ujnCol?.computedAmount}`);
    const totalCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'Total Amount');
    assert(totalCol?.computedAmount === TOTAL, `Total Amount calculated must be ${TOTAL}, got ${totalCol?.computedAmount}`);
}
/** cwra — UJN values directly in UJN column (7 items summing to 951892) + NSI; subtotal row 11. */
function testCwraDirectUjnColumnStep6() {
    const UJN = 951892;
    const NSI = 13400;
    const TOTAL = UJN + NSI;
    const header = { ...THARALI_HEADER };
    const rows = [
        ['S.No', 'Description', 'Unit', 'Qty', 'Rate', 'DSR', 'UJN', 'SOR(PWD)', 'NSI', 'Total Amount'],
        [1, 'Lump sum item A', 'LS', 0, 0, 0, 200000, 0, 0, 0],
        [2, 'Lump sum item B', 'LS', 0, 0, 0, 150000, 0, 0, 0],
        [3, 'Lump sum item C', 'LS', 0, 0, 0, 100000, 0, 0, 0],
        [4, 'Lump sum item D', 'LS', 0, 0, 0, 250000, 0, 0, 0],
        [5, 'Lump sum item E', 'LS', 0, 0, 0, 151892, 0, 0, 0],
        [6, 'Lump sum item F', 'LS', 0, 0, 0, 100000, 0, 0, 0],
        [7, 'NSI rated item', 'LS', 0, 0, 0, 0, 0, NSI, NSI],
        [9, 'Protection works for substructure', '', '', '', 0, 0, 0, 0, 0],
        [11, 'Sub Total', '', '', '', 0, UJN, 0, NSI, TOTAL],
    ];
    const checks = (0, dpr_boq_validation_util_1.validateTotalRows)(rows, header, 1, TOTAL);
    const subTotal = checks.find((c) => c.checkStep === 6 && c.rowNo === 10);
    assert(!!subTotal, 'Expected Step 6 Sub Total check at row 11');
    assert(subTotal.match === true, `Sub Total should pass; got: ${subTotal.message}`);
    const ujnCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'UJN');
    assert(ujnCol?.computedAmount === UJN, `UJN calculated must be ${UJN}, got ${ujnCol?.computedAmount}`);
    const totalCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'Total Amount');
    assert(totalCol?.computedAmount === TOTAL, `Total Amount calculated must be ${TOTAL}, got ${totalCol?.computedAmount}`);
}
/** cwra — Rate column lump sums with incidental Qty=1 (serial leak); subtotal row 11. */
function testCwraRateLumpQtyOneStep6() {
    const UJN = 951892;
    const NSI = 13400;
    const TOTAL = UJN + NSI;
    const header = { ...THARALI_HEADER };
    const rows = [
        ['S.No', 'Description', 'Unit', 'Qty', 'Rate', 'DSR', 'UJN', 'SOR(PWD)', 'NSI', 'Total Amount'],
        [1, 'Lump sum item A', 'LS', 1, 500000, 0, 0, 0, 0, 0],
        [2, 'Lump sum item B', 'LS', 1, 451892, 0, 0, 0, 0, 0],
        [3, 'NSI rated item', 'LS', 0, 0, 0, 0, 0, NSI, NSI],
        [9, 'Protection works for substructure', '', '', '', 0, 0, 0, 0, 0],
        [11, 'Sub Total', '', '', '', 0, UJN, 0, NSI, TOTAL],
    ];
    const checks = (0, dpr_boq_validation_util_1.validateTotalRows)(rows, header, 1, TOTAL);
    const subTotal = checks.find((c) => c.checkStep === 6 && c.rowNo === 6);
    assert(!!subTotal, 'Expected Step 6 Sub Total check at row 11');
    assert(subTotal.match === true, `Sub Total should pass; got: ${subTotal.message}`);
    const ujnCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'UJN');
    assert(ujnCol?.computedAmount === UJN, `UJN calculated must be ${UJN}, got ${ujnCol?.computedAmount}`);
    const totalCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'Total Amount');
    assert(totalCol?.computedAmount === TOTAL, `Total Amount calculated must be ${TOTAL}, got ${totalCol?.computedAmount}`);
}
/** cwra — lump sums in Rate column (LS items) + NSI; subtotal row 11 not row 8. */
function testCwraRateColumnLumpSumStep6() {
    const UJN = 951892;
    const NSI = 13400;
    const TOTAL = UJN + NSI;
    const header = { ...THARALI_HEADER };
    const rows = [
        ['S.No', 'Description', 'Unit', 'Qty', 'Rate', 'DSR', 'UJN', 'SOR(PWD)', 'NSI', 'Total Amount'],
        [1, 'Lump sum item A', 'LS', 0, 500000, 0, 0, 0, 0, 0],
        [2, 'Lump sum item B', 'LS', 0, 451892, 0, 0, 0, 0, 0],
        [3, 'NSI rated item', 'LS', 0, 0, 0, 0, 0, NSI, NSI],
        [9, 'Protection works for substructure', '', '', '', 0, 0, 0, 0, 0],
        [11, 'Sub Total', '', '', '', 0, UJN, 0, NSI, TOTAL],
    ];
    const checks = (0, dpr_boq_validation_util_1.validateTotalRows)(rows, header, 1, TOTAL);
    const subTotal = checks.find((c) => c.checkStep === 6 && c.rowNo === 6);
    assert(!!subTotal, 'Expected Step 6 Sub Total check at row 11');
    assert(subTotal.match === true, `Sub Total should pass; got: ${subTotal.message}`);
    const ujnCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'UJN');
    assert(ujnCol?.computedAmount === UJN, `UJN calculated must be ${UJN}, got ${ujnCol?.computedAmount}`);
    const totalCol = subTotal.columnChecks?.find((c) => c.columnLabel === 'Total Amount');
    assert(totalCol?.computedAmount === TOTAL, `Total Amount calculated must be ${TOTAL}, got ${totalCol?.computedAmount}`);
}
/** cwra row 8 wrongly labeled Sub Total — must contribute as item; real subtotal row 11. */
function testCwraPrematureSubTotalRow8() {
    const UJN = 951892;
    const NSI = 13400;
    const SOR1 = 57917.48;
    const SOR2 = 38611.65;
    const SOR = 96529.13;
    const TOTAL = 965292;
    const header = { ...THARALI_HEADER };
    const rows = [
        ['S.No', 'Description', 'Unit', 'Qty', 'Rate', 'DSR', 'UJN', 'SOR(PWD)', 'NSI', 'Total Amount'],
        [8, 'Sub Total', '', '', '', 0, UJN, 0, NSI, UJN + NSI],
        [9, 'Protection works for substructure and foundation', '', '', '', 0, 0, 0, 0, 0],
        [10, 'SOR rated item A', 'Nos', 1, SOR1, 0, 0, SOR1, 0, SOR1],
        [10, 'SOR rated item B', 'Nos', 1, SOR2, 0, 0, SOR2, 0, SOR2],
        [11, 'Sub Total', '', '', '', 0, UJN, SOR, NSI, TOTAL],
    ];
    const checks = (0, dpr_boq_validation_util_1.validateTotalRows)(rows, header, 1, TOTAL);
    const subTotal = checks.find((c) => c.checkStep === 6 && c.rowNo === 6);
    assert(!!subTotal, 'Expected Step 6 Sub Total check at row 11 only');
    assert(subTotal.match === true, `Sub Total should pass; got: ${subTotal.message}`);
    const premature = checks.find((c) => c.checkStep === 6 && c.rowNo === 2);
    assert(!premature, 'Row 8 must not trigger Step 6 when row 10 items follow');
}
/** so & Tra — UJN ₹4 rounding slack (summed 1300 vs Excel 1296) must pass Step 6/7. */
function testSoTraUjnTolerance1296vs1300() {
    const UJN_EXCEL = 1296;
    const UJN_SUMMED = 1300;
    const TOTAL = UJN_EXCEL;
    const rows = [
        ['S.No', 'Description', 'Unit', 'Qty', 'Rate', 'DSR', 'UJN', 'SOR(PWD)', 'NSI', 'Total Amount'],
        [1, 'Small UJN item', 'Nos', 1, UJN_SUMMED, 0, UJN_SUMMED, 0, 0, UJN_SUMMED],
        [2, 'Sub Total', '', '', '', 0, UJN_EXCEL, 0, 0, TOTAL],
        [11, '', 'Total Cost', '', '', 0, UJN_EXCEL, 0, 0, TOTAL],
    ];
    const checks = (0, dpr_boq_validation_util_1.validateTotalRows)(rows, THARALI_HEADER, 1, TOTAL);
    const subTotal = checks.find((c) => c.checkStep === 6);
    assert(!!subTotal, 'Expected Step 6 Sub Total check');
    assert(subTotal.match === true, `Sub Total should pass within ₹5 tolerance; got: ${subTotal.message}`);
    const ujnStep6 = subTotal.columnChecks?.find((c) => c.columnLabel === 'UJN');
    assert(ujnStep6?.declaredAmount === UJN_EXCEL, `UJN declared must be ${UJN_EXCEL}`);
    assert(ujnStep6?.computedAmount === UJN_SUMMED, `UJN computed must be ${UJN_SUMMED}`);
    assert(ujnStep6?.match === true, 'Step 6 UJN 1296 vs 1300 must pass within ₹5 tolerance');
    const totalCost = checks.find((c) => c.checkStep === 7 && c.rowNo === 4);
    assert(!!totalCost, 'Expected Step 7 Total Cost check at row 4');
    assert(totalCost.match === true, `Total Cost should pass; got: ${totalCost.message}`);
}
function runAll() {
    testSoTraStep7TotalCostRow11();
    testSoTraUjnTolerance1296vs1300();
    testCwraAmountOnlyRowsStep6();
    testCwraRows8_10_11Step6();
    testCwraPrematureSubTotalRow8();
    testCwraUnmappedAmtColumnStep6();
    testCwraDirectUjnColumnStep6();
    testCwraRateLumpQtyOneStep6();
    testCwraRateColumnLumpSumStep6();
    console.log('dpr-boq-section-totals: all tests passed');
}
runAll();
