...
for mig in 096_project_deletion_ee_approval.sql 097_secretariat_dpr_role.sql 098_secretariat_stage8_dpr_update.sql \
  ...
  108_om_submit_permissions.sql 109_om_handover_demo_permissions.sql 110_contractor_map_handover_access.sql \
  111_tharali_handover_je_inbox.sql 112_handover_je_inbox_repair.sql; do
  if [[ -f "${ROOT}/database/migrations/${mig}" ]]; then
    echo "    Applying ${mig}..."
    if ! "${COMPOSE[@]}" exec -T postgres psql -U egip -d egip -v ON_ERROR_STOP=1 \
      < "${ROOT}/database/migrations/${mig}"; then
      echo "ERROR: migration ${mig} failed — fix DB before continuing"
      exit 1
    fi
  fi
done
...
echo "==> 8b. Karanprayag JE + handover workflow repair"
if [[ -f "${ROOT}/database/scripts/vps-fix-kpg-je-handover.sql" ]]; then
  "${COMPOSE[@]}" exec -T postgres psql -U egip -d egip -v ON_ERROR_STOP=1 \
    < "${ROOT}/database/scripts/vps-fix-kpg-je-handover.sql" || echo "WARN: KPG JE handover fix failed"
fi
