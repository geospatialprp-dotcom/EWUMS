-- VPS: backfill Karanprayag Audit Trail so Admin (KPG filter) and EE panel match.
-- Run:
--   docker compose -f docker-compose.prod.yml --env-file .env exec -T postgres \
--     psql -U egip -d egip -f - < /opt/egip/database/migrations/119_kpg_audit_trail_backfill.sql
--
-- Or paste this whole file into psql.
...
BEGIN
  ...
  IF v_ee IS NULL AND v_je IS NULL THEN
    RAISE NOTICE 'No KPG EE/JE users — skip';
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'Karanprayag audit trail backfill complete';
END $$;

SELECT COUNT(*) AS kpg_audit_rows
FROM audit_logs
WHERE details->>'divisionId' = 'd1000000-0000-0000-0000-000000000010';
