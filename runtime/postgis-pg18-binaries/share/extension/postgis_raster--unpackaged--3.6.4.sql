...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
--
-- Generated on: 2026-06-08 21:54:09
...
BEGIN
	...
	sql := pg_catalog.format('ALTER EXTENSION postgis_raster ADD %s', obj);
	EXECUTE sql;
...
SELECT _postgis_package_object('AGGREGATE', 'ST_AsRasterAgg (geom geometry, val float8, ref raster, pixeltype text, nodataval float8, uniontype text, touched boolean)');
...
SELECT _postgis_package_object('FUNCTION', 'ST_ReclassExact (rast raster,
	inputvalues float8[], outputvalues float8[],
	bandnumber integer ,
	outputpixeltype text ,
	nodatavalue float8 )');
...
SELECT _postgis_package_object('FUNCTION', 'ST_IntersectionFractions (rast raster, geom geometry)');
...
SELECT _postgis_package_object('FUNCTION', '_ST_AsRasterAgg_transfn (staterast raster, geom geometry, val float8, rast raster, pixeltype text, nodataval float8, uniontype text, touched boolean)');
SELECT _postgis_package_object('FUNCTION', '_ST_AsRasterAgg_finalfn (rast raster)');
...
-- drop st_bytea
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP CAST IF EXISTS (raster AS bytea)');
...
-- drop box2d
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP CAST IF EXISTS (raster AS box2d)');
DROP CAST IF EXISTS (raster AS box2d);
SELECT _postgis_drop_function_by_signature('box2d(raster)');

-- make geometry cast ASSIGNMENT
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP CAST IF EXISTS (raster AS geometry)');
...
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP TYPE IF EXISTS quantile');
...
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP TYPE IF EXISTS valuecount');
...
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP TYPE IF EXISTS histogram');
DROP TYPE IF EXISTS histogram;

-- no longer needed functions changed to use out parameters
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP TYPE IF EXISTS bandmetadata');
DROP TYPE IF EXISTS bandmetadata;
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP TYPE IF EXISTS geomvalxy');
...
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP FUNCTION _rename_raster_tables()');
DROP FUNCTION _rename_raster_tables();

-- inserted new column into view
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP VIEW IF EXISTS raster_columns');
...
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP FUNCTION _drop_st_samealignment()');
...
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP TYPE IF EXISTS wktgeomval');
...
CREATE OR REPLACE FUNCTION bytea(raster)
    ...
    AS '$libdir/postgis_raster-3', 'RASTER_to_bytea'
    LANGUAGE 'c' IMMUTABLE STRICT PARALLEL SAFE;
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP CAST IF EXISTS (raster AS box3d)');
...
DECLARE
  tmprast RASTER;
BEGIN
  tmprast := @extschema@.ST_AsRaster(geom, rast,
    pixeltype => pixeltype,
    value => val,
    nodataval => nodataval,
    touched => touched
  ...
  IF staterast IS NULL THEN
    staterast = tmprast;
  ELSE
    SELECT @extschema@.ST_Union(r, uniontype)
...
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP FUNCTION _postgis_upgrade_info()');
...
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP AGGREGATE IF EXISTS ST_Union(raster, text, text, text, double precision, text, text, text, double precision, text, text, text, double precision)');
...
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP AGGREGATE IF EXISTS ST_Union(raster, text, text, text)');
DROP AGGREGATE IF EXISTS ST_Union(raster, text, text, text);
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP AGGREGATE IF EXISTS ST_Union(raster, text, text, text, double precision, text, text, text, double precision)');
DROP AGGREGATE IF EXISTS ST_Union(raster, text, text, text, double precision, text, text, text, double precision);
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP AGGREGATE IF EXISTS ST_Union(raster, text, text)');
DROP AGGREGATE IF EXISTS ST_Union(raster, text, text);
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP AGGREGATE IF EXISTS ST_Union(raster, text, text, text, double precision)');
DROP AGGREGATE IF EXISTS ST_Union(raster, text, text, text, double precision);
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP AGGREGATE IF EXISTS ST_Union(raster, record[])');
...
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP FUNCTION _postgis_drop_function_by_identity(text, text, text)');
DROP FUNCTION _postgis_drop_function_by_identity(text, text, text);
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP FUNCTION _postgis_drop_function_by_signature(text, text)');
DROP FUNCTION _postgis_drop_function_by_signature(text, text);
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP FUNCTION _postgis_topology_upgrade_domain_type(text, text, text, text)');
DROP FUNCTION _postgis_topology_upgrade_domain_type(text, text, text, text);
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP FUNCTION _postgis_drop_cast_by_types(text, text, text)');
DROP FUNCTION _postgis_drop_cast_by_types(text, text, text);
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP FUNCTION _postgis_topology_upgrade_user_type_attribute(text, text, text, text, text)');
DROP FUNCTION _postgis_topology_upgrade_user_type_attribute(text, text, text, text, text);
SELECT @extschema@.postgis_extension_drop_if_exists('postgis_raster', 'DROP FUNCTION _postgis_add_column_to_table(regclass, name, text, boolean, text, text)');
