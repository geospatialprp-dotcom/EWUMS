...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
--
-- Generated on: 2026-06-08 21:54:12
...
SELECT _postgis_package_object('FUNCTION', 'CG_3DBuffer (geom geometry, radius float8, segments integer, buffer_type integer)');
SELECT _postgis_package_object('FUNCTION', 'CG_StraightSkeletonPartition (geom geometry, auto_orientation boolean)');
SELECT _postgis_package_object('FUNCTION', 'CG_RotateX (geom geometry, angle float8)');
SELECT _postgis_package_object('FUNCTION', 'CG_RotateY (geom geometry, angle float8)');
SELECT _postgis_package_object('FUNCTION', 'CG_RotateZ (geom geometry, angle float8)');
SELECT _postgis_package_object('FUNCTION', 'CG_Rotate (geom geometry, angle float8)');
SELECT _postgis_package_object('FUNCTION', 'CG_2DRotate (geom geometry, angle float8, cx float8, cy float8)');
SELECT _postgis_package_object('FUNCTION', 'CG_3DRotate (geom geometry, angle float8, ax float8, ay float8, az float8)');
SELECT _postgis_package_object('FUNCTION', 'CG_Scale (geom geometry, factor float8)');
SELECT _postgis_package_object('FUNCTION', 'CG_3DScaleAroundCenter (geom geometry, factorX float8, factorY float8, factorZ float8, centerX float8, centerY float8, centerZ float8)');
SELECT _postgis_package_object('FUNCTION', 'CG_3DScale (geom geometry, factorX float8, factorY float8, factorZ float8)');
SELECT _postgis_package_object('FUNCTION', 'CG_Translate (geom geometry, deltaX float8, deltaY float8)');
SELECT _postgis_package_object('FUNCTION', 'CG_3DTranslate (geom geometry, deltaX float8, deltaY float8, deltaZ float8)');
SELECT _postgis_package_object('FUNCTION', 'CG_Simplify (geom geometry, threshold float8, preserveTopology boolean )');
SELECT _postgis_package_object('FUNCTION', 'CG_3DAlphaWrapping (geom geometry, relative_alpha int, relative_offset int )');
-- Register all types.
