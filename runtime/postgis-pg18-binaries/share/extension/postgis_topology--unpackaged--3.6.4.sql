...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
--
-- Generated on: 2026-06-08 21:54:10
...
BEGIN
	...
	sql := pg_catalog.format('ALTER EXTENSION postgis_topology ADD %s', obj);
	EXECUTE sql;
...
SELECT _postgis_package_object('CAST', '(topology.TopoGeometry AS bigint[])');
...
SELECT _postgis_package_object('FUNCTION', 'topology.CreateTopoGeom (toponame varchar, tg_type integer, layer_id integer, tg_objs topology.TopoElementArray, tg_id bigint )');
...
SELECT _postgis_package_object('FUNCTION', 'topology.GetTopoGeomElementArray (toponame varchar, layer_id integer, tgid bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology.GetTopoGeomElementArray (tg topology.TopoGeometry)');
SELECT _postgis_package_object('FUNCTION', 'topology.GetTopoGeomElements (toponame varchar, layerid integer, tgid bigint)');
...
SELECT _postgis_package_object('FUNCTION', 'topology._st_mintolerance (val float8)');
...
SELECT _postgis_package_object('FUNCTION', 'topology._AsGMLNode (id bigint, point geometry, nsprefix_in text, prec int, options int, idprefix text, gmlver int)');
SELECT _postgis_package_object('FUNCTION', 'topology._AsGMLEdge (edge_id bigint, start_node bigint, end_node bigint, line geometry, visitedTable regclass, nsprefix_in text, prec int, options int, idprefix text, gmlver int)');
SELECT _postgis_package_object('FUNCTION', 'topology._AsGMLFace (toponame text, face_id bigint, visitedTable regclass, nsprefix_in text, prec int, options int, idprefix text, gmlver int)');
...
SELECT _postgis_package_object('FUNCTION', 'topology.ST_GetFaceEdges (toponame varchar, face_id bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_NewEdgeHeal (toponame varchar, e1id bigint, e2id bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_ModEdgeHeal (toponame varchar, e1id bigint, e2id bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_RemEdgeNewFace (toponame varchar, e1id bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_RemEdgeModFace (toponame varchar, e1id bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_GetFaceGeometry (toponame varchar, aface bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_AddIsoNode (atopology varchar, aface bigint, apoint geometry)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_MoveIsoNode (atopology character varying, anode bigint, apoint geometry)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_RemoveIsoNode (atopology varchar, anode bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_RemIsoNode (varchar, bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_RemoveIsoEdge (atopology varchar, anedge bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_NewEdgesSplit (atopology varchar, anedge bigint, apoint geometry)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_ModEdgeSplit (atopology varchar, anedge bigint, apoint geometry)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_AddIsoEdge (atopology varchar, anode bigint, anothernode bigint, acurve geometry)');
SELECT _postgis_package_object('FUNCTION', 'topology._ST_AdjacentEdges (atopology varchar, anode bigint, anedge bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_ChangeEdgeGeom (atopology varchar, anedge bigint, acurve geometry)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_AddEdgeNewFaces (atopology varchar, anode bigint, anothernode bigint, acurve geometry)');
SELECT _postgis_package_object('FUNCTION', 'topology.ST_AddEdgeModFace (atopology varchar, anode bigint, anothernode bigint, acurve geometry)');
...
SELECT _postgis_package_object('FUNCTION', 'topology.GetRingEdges (atopology varchar, anedge bigint, maxedges int )');
SELECT _postgis_package_object('FUNCTION', 'topology.GetNodeEdges (atopology varchar, anode bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology.AddToSearchPath (a_schema_name varchar)');
SELECT _postgis_package_object('FUNCTION', 'topology.AddTopoGeometryColumn (toponame name, tab regclass, col name, layerid integer, layertype varchar, child integer )');
SELECT _postgis_package_object('FUNCTION', 'topology.AddTopoGeometryColumn (toponame name, schema name, tbl name, col name, ltype varchar, child integer )');
...
SELECT _postgis_package_object('FUNCTION', 'topology.CreateTopology (atopology name, srid integer , prec float8 , hasZ boolean , topoid integer , usesLargeIDs boolean )');
...
SELECT _postgis_package_object('FUNCTION', 'topology.FixCorruptTopoGeometryColumn (layerSchema name, layerTable name, layerColumn name)');
...
SELECT _postgis_package_object('FUNCTION', 'topology._validatetopologygetfaceshellmaximaledgering (atopology varchar, aface bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology._ValidateTopologyGetRingEdges (starting_edge bigint)');
SELECT _postgis_package_object('FUNCTION', 'topology._CheckEdgeLinking (curedge_edge_id BIGINT, prevedge_edge_id BIGINT, prevedge_next_left_edge BIGINT, prevedge_next_right_edge BIGINT)');
...
SELECT _postgis_package_object('FUNCTION', 'topology.ValidateTopologyPrecision (toponame name, bbox geometry , gridSize float8 )');
SELECT _postgis_package_object('FUNCTION', 'topology.MakeTopologyPrecise (toponame name, bbox GEOMETRY , gridSize float8 )');
SELECT _postgis_package_object('FUNCTION', 'topology.TotalTopologySize (toponame name)');
SELECT _postgis_package_object('FUNCTION', 'topology.UpgradeTopology (toponame name)');
...
--
-- UPGRADE SCRIPT TO PostGIS 3.6.4
--
...
BEGIN
    ...
    old_scripts := postgis_topology_scripts_installed();
    new_scripts := '3.6.4';
    ...
    THEN
        RAISE EXCEPTION 'Downgrade of postgis_topology from version % to version % is forbidden', old_scripts, new_scripts;
    END IF;
    ...
    IF old_maj != new_maj THEN
        RAISE EXCEPTION 'Upgrade of postgis_topology from version % to version % requires a dump/reload. See PostGIS manual for instructions', old_scripts, new_scripts;
    END IF;

    WITH versions AS (
      SELECT '3.6.4'::text as upgraded,
      postgis_topology_scripts_installed() as installed
    ) SELECT
...
-- Alter Type Attribute from integer to bigint -- LastUpdated: 101
SELECT _postgis_topology_upgrade_user_type_attribute('topogeometry', 'id', 'integer', 'bigint', '306');





-- Domain: topology.topoelement(bigint[])
-- topology.topoelement(integer[]) -- LastUpdated: 306
-- Updated Domain topology.topoelement(bigint[])
-- We cannot drop the old domain, so we modify it
DO LANGUAGE 'plpgsql'
$postgis_domain_upgrade$
BEGIN
IF 306 > version_from_num
    FROM _postgis_upgrade_info()
THEN
    PERFORM _postgis_topology_upgrade_domain_type('topoelement', 'integer[]', 'bigint[]', '306');
END IF;
END
$postgis_domain_upgrade$;

ALTER DOMAIN topology.topoelement DROP CONSTRAINT IF EXISTS DIMENSIONS;
ALTER DOMAIN topology.topoelement ADD CONSTRAINT DIMENSIONS CHECK ( array_upper(VALUE, 2) IS NULL AND array_upper(VALUE, 1) = 2 );

ALTER DOMAIN topology.topoelement DROP CONSTRAINT IF EXISTS type_range;
ALTER DOMAIN topology.topoelement ADD CONSTRAINT type_range CHECK ( VALUE[2] > 0 );

ALTER DOMAIN topology.topoelement DROP CONSTRAINT IF EXISTS lower_dimension;
ALTER DOMAIN topology.topoelement ADD CONSTRAINT lower_dimension CHECK ( array_lower(VALUE, 1) = 1 );



-- Domain: topology.topoelementarray(bigint[][])
-- topology.topoelementarray(integer[][]) -- LastUpdated: 306
-- Updated Domain topology.topoelementarray(bigint[][])
-- We cannot drop the old domain, so we modify it
DO LANGUAGE 'plpgsql'
$postgis_domain_upgrade$
BEGIN
IF 306 > version_from_num
    FROM _postgis_upgrade_info()
THEN
    PERFORM _postgis_topology_upgrade_domain_type('topoelementarray', 'integer[][]', 'bigint[][]', '306');
END IF;
END
$postgis_domain_upgrade$;

ALTER DOMAIN topology.topoelementarray DROP CONSTRAINT IF EXISTS type_range;
ALTER DOMAIN topology.topoelementarray ADD CONSTRAINT type_range CHECK ( array_upper(VALUE, 2) = 2 AND array_upper(VALUE, 3) IS NULL );
...
-- Rename createtopogeom ( varchar, integer, integer, topoelementarray ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'createtopogeom(varchar, integer, integer, topoelementarray)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function createtopogeom(varchar, integer, integer, topoelementarray) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function createtopogeom(varchar, integer, integer, topoelementarray) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    BEGIN
        ALTER FUNCTION createtopogeom( varchar, integer, integer, topoelementarray ) RENAME TO createtopogeom_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function createtopogeom(varchar, integer, integer, topoelementarray) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function createtopogeom(varchar, integer, integer, topoelementarray) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
-- Rename gettopogeomelementarray ( varchar, integer, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'gettopogeomelementarray(varchar, integer, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function gettopogeomelementarray(varchar, integer, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function gettopogeomelementarray(varchar, integer, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION gettopogeomelementarray( varchar, integer, integer ) RENAME TO gettopogeomelementarray_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function gettopogeomelementarray(varchar, integer, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function gettopogeomelementarray(varchar, integer, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
$$
LANGUAGE 'plpgsql' STABLE STRICT;
-- Rename gettopogeomelements ( varchar, integer, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'gettopogeomelements(varchar, integer, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function gettopogeomelements(varchar, integer, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function gettopogeomelements(varchar, integer, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION gettopogeomelements( varchar, integer, integer ) RENAME TO gettopogeomelements_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function gettopogeomelements(varchar, integer, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function gettopogeomelements(varchar, integer, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
DROP CAST IF EXISTS (topology.TopoGeometry AS bigint[]);
...
$$
LANGUAGE 'plpgsql' STABLE STRICT;
-- Rename getnodebypoint ( varchar, geometry, float8 ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'getnodebypoint(varchar, geometry, float8)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function getnodebypoint(varchar, geometry, float8) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function getnodebypoint(varchar, geometry, float8) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION getnodebypoint( varchar, geometry, float8 ) RENAME TO getnodebypoint_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function getnodebypoint(varchar, geometry, float8) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function getnodebypoint(varchar, geometry, float8) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
-- Rename getedgebypoint ( varchar, geometry, float8 ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'getedgebypoint(varchar, geometry, float8)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function getedgebypoint(varchar, geometry, float8) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function getedgebypoint(varchar, geometry, float8) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION getedgebypoint( varchar, geometry, float8 ) RENAME TO getedgebypoint_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function getedgebypoint(varchar, geometry, float8) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function getedgebypoint(varchar, geometry, float8) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.GetEdgeByPoint(atopology varchar, apoint geometry, tol1 float8)
	...
	LANGUAGE 'c' STABLE STRICT;
-- Rename getfacebypoint ( varchar, geometry, float8 ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'getfacebypoint(varchar, geometry, float8)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function getfacebypoint(varchar, geometry, float8) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function getfacebypoint(varchar, geometry, float8) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION getfacebypoint( varchar, geometry, float8 ) RENAME TO getfacebypoint_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function getfacebypoint(varchar, geometry, float8) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function getfacebypoint(varchar, geometry, float8) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
-- Rename getfacecontainingpoint ( text, geometry ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'getfacecontainingpoint(text, geometry)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function getfacecontainingpoint(text, geometry) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function getfacecontainingpoint(text, geometry) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION getfacecontainingpoint( text, geometry ) RENAME TO getfacecontainingpoint_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function getfacecontainingpoint(text, geometry) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function getfacecontainingpoint(text, geometry) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
END;
$$ LANGUAGE 'plpgsql' STABLE STRICT;
-- Rename addnode ( varchar, geometry, boolean, boolean ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'addnode(varchar, geometry, boolean, boolean)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function addnode(varchar, geometry, boolean, boolean) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function addnode(varchar, geometry, boolean, boolean) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION addnode( varchar, geometry, boolean, boolean ) RENAME TO addnode_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function addnode(varchar, geometry, boolean, boolean) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function addnode(varchar, geometry, boolean, boolean) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
$$
LANGUAGE 'plpgsql' VOLATILE;
-- Rename addedge ( varchar, geometry ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'addedge(varchar, geometry)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function addedge(varchar, geometry) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function addedge(varchar, geometry) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION addedge( varchar, geometry ) RENAME TO addedge_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function addedge(varchar, geometry) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function addedge(varchar, geometry) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
$$
LANGUAGE 'plpgsql' VOLATILE;
-- Rename addface ( varchar, geometry, boolean ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'addface(varchar, geometry, boolean)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function addface(varchar, geometry, boolean) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function addface(varchar, geometry, boolean) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION addface( varchar, geometry, boolean ) RENAME TO addface_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function addface(varchar, geometry, boolean) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function addface(varchar, geometry, boolean) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
$$
LANGUAGE 'plpgsql' VOLATILE;
-- Rename topogeo_addpoint ( varchar, geometry, float8 ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'topogeo_addpoint(varchar, geometry, float8)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function topogeo_addpoint(varchar, geometry, float8) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function topogeo_addpoint(varchar, geometry, float8) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION topogeo_addpoint( varchar, geometry, float8 ) RENAME TO topogeo_addpoint_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function topogeo_addpoint(varchar, geometry, float8) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function topogeo_addpoint(varchar, geometry, float8) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.TopoGeo_AddPoint(atopology varchar, apoint geometry, tolerance float8 DEFAULT 0)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename topogeo_addlinestring ( varchar, geometry, float8 ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'topogeo_addlinestring(varchar, geometry, float8)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function topogeo_addlinestring(varchar, geometry, float8) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function topogeo_addlinestring(varchar, geometry, float8) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION topogeo_addlinestring( varchar, geometry, float8 ) RENAME TO topogeo_addlinestring_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function topogeo_addlinestring(varchar, geometry, float8) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function topogeo_addlinestring(varchar, geometry, float8) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.TopoGeo_AddLinestring(atopology varchar, aline geometry, tolerance float8 DEFAULT 0)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename topogeo_addpolygon ( varchar, geometry, float8 ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'topogeo_addpolygon(varchar, geometry, float8)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function topogeo_addpolygon(varchar, geometry, float8) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function topogeo_addpolygon(varchar, geometry, float8) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION topogeo_addpolygon( varchar, geometry, float8 ) RENAME TO topogeo_addpolygon_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function topogeo_addpolygon(varchar, geometry, float8) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function topogeo_addpolygon(varchar, geometry, float8) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
$BODY$
LANGUAGE 'plpgsql' VOLATILE STRICT;
-- Rename _asgmlnode ( integer, geometry, text, integer, integer, text, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = '_asgmlnode(integer, geometry, text, integer, integer, text, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _asgmlnode(integer, geometry, text, integer, integer, text, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function _asgmlnode(integer, geometry, text, integer, integer, text, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION _asgmlnode( integer, geometry, text, integer, integer, text, integer ) RENAME TO _asgmlnode_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _asgmlnode(integer, geometry, text, integer, integer, text, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function _asgmlnode(integer, geometry, text, integer, integer, text, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
-- Rename _asgmledge ( integer, integer, integer, geometry, regclass, text, integer, integer, text, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = '_asgmledge(integer, integer, integer, geometry, regclass, text, integer, integer, text, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _asgmledge(integer, integer, integer, geometry, regclass, text, integer, integer, text, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function _asgmledge(integer, integer, integer, geometry, regclass, text, integer, integer, text, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION _asgmledge( integer, integer, integer, geometry, regclass, text, integer, integer, text, integer ) RENAME TO _asgmledge_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _asgmledge(integer, integer, integer, geometry, regclass, text, integer, integer, text, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function _asgmledge(integer, integer, integer, geometry, regclass, text, integer, integer, text, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
-- Rename _asgmlface ( text, integer, regclass, text, integer, integer, text, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = '_asgmlface(text, integer, regclass, text, integer, integer, text, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _asgmlface(text, integer, regclass, text, integer, integer, text, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function _asgmlface(text, integer, regclass, text, integer, integer, text, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION _asgmlface( text, integer, regclass, text, integer, integer, text, integer ) RENAME TO _asgmlface_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _asgmlface(text, integer, regclass, text, integer, integer, text, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function _asgmlface(text, integer, regclass, text, integer, integer, text, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
$$ LANGUAGE 'plpgsql' VOLATILE; -- writes into visited table
-- Alter Type Attribute from integer to bigint -- LastUpdated: 101
SELECT _postgis_topology_upgrade_user_type_attribute('getfaceedges_returntype', 'edge', 'integer', 'bigint', '306');



-- Rename st_getfaceedges ( varchar, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_getfaceedges(varchar, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_getfaceedges(varchar, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_getfaceedges(varchar, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_getfaceedges( varchar, integer ) RENAME TO st_getfaceedges_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_getfaceedges(varchar, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_getfaceedges(varchar, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_GetFaceEdges(toponame varchar, face_id bigint)
  ...
  LANGUAGE 'c' STABLE;
-- Rename st_newedgeheal ( varchar, integer, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_newedgeheal(varchar, integer, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_newedgeheal(varchar, integer, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_newedgeheal(varchar, integer, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_newedgeheal( varchar, integer, integer ) RENAME TO st_newedgeheal_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_newedgeheal(varchar, integer, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_newedgeheal(varchar, integer, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_NewEdgeHeal(toponame varchar, e1id bigint, e2id bigint)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename st_modedgeheal ( varchar, integer, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_modedgeheal(varchar, integer, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_modedgeheal(varchar, integer, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_modedgeheal(varchar, integer, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_modedgeheal( varchar, integer, integer ) RENAME TO st_modedgeheal_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_modedgeheal(varchar, integer, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_modedgeheal(varchar, integer, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_ModEdgeHeal(toponame varchar, e1id bigint, e2id bigint)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename st_remedgenewface ( varchar, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_remedgenewface(varchar, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_remedgenewface(varchar, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_remedgenewface(varchar, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_remedgenewface( varchar, integer ) RENAME TO st_remedgenewface_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_remedgenewface(varchar, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_remedgenewface(varchar, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_RemEdgeNewFace(toponame varchar, e1id bigint)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename st_remedgemodface ( varchar, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_remedgemodface(varchar, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_remedgemodface(varchar, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_remedgemodface(varchar, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_remedgemodface( varchar, integer ) RENAME TO st_remedgemodface_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_remedgemodface(varchar, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_remedgemodface(varchar, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_RemEdgeModFace(toponame varchar, e1id bigint)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename st_getfacegeometry ( varchar, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_getfacegeometry(varchar, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_getfacegeometry(varchar, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_getfacegeometry(varchar, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_getfacegeometry( varchar, integer ) RENAME TO st_getfacegeometry_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_getfacegeometry(varchar, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_getfacegeometry(varchar, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_GetFaceGeometry(toponame varchar, aface bigint)
  ...
  LANGUAGE 'c' STABLE;
-- Rename st_addisonode ( varchar, integer, geometry ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_addisonode(varchar, integer, geometry)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_addisonode(varchar, integer, geometry) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_addisonode(varchar, integer, geometry) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_addisonode( varchar, integer, geometry ) RENAME TO st_addisonode_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_addisonode(varchar, integer, geometry) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_addisonode(varchar, integer, geometry) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_AddIsoNode(atopology varchar, aface bigint, apoint geometry)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename st_moveisonode ( varchar, integer, geometry ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_moveisonode(varchar, integer, geometry)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_moveisonode(varchar, integer, geometry) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_moveisonode(varchar, integer, geometry) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_moveisonode( varchar, integer, geometry ) RENAME TO st_moveisonode_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_moveisonode(varchar, integer, geometry) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_moveisonode(varchar, integer, geometry) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_MoveIsoNode(atopology character varying, anode bigint, apoint geometry)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename st_removeisonode ( varchar, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_removeisonode(varchar, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_removeisonode(varchar, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_removeisonode(varchar, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_removeisonode( varchar, integer ) RENAME TO st_removeisonode_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_removeisonode(varchar, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_removeisonode(varchar, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_RemoveIsoNode(atopology varchar, anode bigint)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename st_remisonode ( varchar, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_remisonode(varchar, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_remisonode(varchar, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_remisonode(varchar, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_remisonode( varchar, integer ) RENAME TO st_remisonode_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_remisonode(varchar, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_remisonode(varchar, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_RemIsoNode(varchar, bigint)
  RETURNS TEXT AS
	'$libdir/postgis_topology-3','ST_RemoveIsoNode'
  LANGUAGE 'c' VOLATILE;
-- Rename st_removeisoedge ( varchar, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_removeisoedge(varchar, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_removeisoedge(varchar, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_removeisoedge(varchar, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_removeisoedge( varchar, integer ) RENAME TO st_removeisoedge_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_removeisoedge(varchar, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_removeisoedge(varchar, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_RemoveIsoEdge(atopology varchar, anedge bigint)
  RETURNS TEXT AS
	...
	LANGUAGE 'c' VOLATILE;
-- Rename st_newedgessplit ( varchar, integer, geometry ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_newedgessplit(varchar, integer, geometry)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_newedgessplit(varchar, integer, geometry) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_newedgessplit(varchar, integer, geometry) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_newedgessplit( varchar, integer, geometry ) RENAME TO st_newedgessplit_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_newedgessplit(varchar, integer, geometry) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_newedgessplit(varchar, integer, geometry) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
END;
$postgis_proc_upgrade$;
-- Rename st_modedgesplit ( varchar, integer, geometry ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_modedgesplit(varchar, integer, geometry)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_modedgesplit(varchar, integer, geometry) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_modedgesplit(varchar, integer, geometry) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_modedgesplit( varchar, integer, geometry ) RENAME TO st_modedgesplit_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_modedgesplit(varchar, integer, geometry) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_modedgesplit(varchar, integer, geometry) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_ModEdgeSplit(atopology varchar, anedge bigint, apoint geometry)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename st_addisoedge ( varchar, integer, integer, geometry ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_addisoedge(varchar, integer, integer, geometry)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_addisoedge(varchar, integer, integer, geometry) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_addisoedge(varchar, integer, integer, geometry) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_addisoedge( varchar, integer, integer, geometry ) RENAME TO st_addisoedge_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_addisoedge(varchar, integer, integer, geometry) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_addisoedge(varchar, integer, integer, geometry) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_AddIsoEdge(atopology varchar, anode bigint, anothernode bigint, acurve geometry)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename _st_adjacentedges ( varchar, integer, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = '_st_adjacentedges(varchar, integer, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _st_adjacentedges(varchar, integer, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function _st_adjacentedges(varchar, integer, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION _st_adjacentedges( varchar, integer, integer ) RENAME TO _st_adjacentedges_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _st_adjacentedges(varchar, integer, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function _st_adjacentedges(varchar, integer, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
-- Rename st_changeedgegeom ( varchar, integer, geometry ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_changeedgegeom(varchar, integer, geometry)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_changeedgegeom(varchar, integer, geometry) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_changeedgegeom(varchar, integer, geometry) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_changeedgegeom( varchar, integer, geometry ) RENAME TO st_changeedgegeom_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_changeedgegeom(varchar, integer, geometry) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_changeedgegeom(varchar, integer, geometry) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_ChangeEdgeGeom(atopology varchar, anedge bigint, acurve geometry)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename st_addedgenewfaces ( varchar, integer, integer, geometry ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_addedgenewfaces(varchar, integer, integer, geometry)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_addedgenewfaces(varchar, integer, integer, geometry) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_addedgenewfaces(varchar, integer, integer, geometry) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_addedgenewfaces( varchar, integer, integer, geometry ) RENAME TO st_addedgenewfaces_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_addedgenewfaces(varchar, integer, integer, geometry) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_addedgenewfaces(varchar, integer, integer, geometry) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.ST_AddEdgeNewFaces(atopology varchar, anode bigint, anothernode bigint, acurve geometry)
  ...
  LANGUAGE 'c' VOLATILE;
-- Rename st_addedgemodface ( varchar, integer, integer, geometry ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_addedgemodface(varchar, integer, integer, geometry)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_addedgemodface(varchar, integer, integer, geometry) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_addedgemodface(varchar, integer, integer, geometry) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_addedgemodface( varchar, integer, integer, geometry ) RENAME TO st_addedgemodface_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_addedgemodface(varchar, integer, integer, geometry) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_addedgemodface(varchar, integer, integer, geometry) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
$$
LANGUAGE 'plpgsql' VOLATILE;
-- Rename getringedges ( varchar, integer, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'getringedges(varchar, integer, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function getringedges(varchar, integer, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function getringedges(varchar, integer, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION getringedges( varchar, integer, integer ) RENAME TO getringedges_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function getringedges(varchar, integer, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function getringedges(varchar, integer, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
LANGUAGE 'c' STABLE;
-- Rename getnodeedges ( varchar, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'getnodeedges(varchar, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function getnodeedges(varchar, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function getnodeedges(varchar, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION getnodeedges( varchar, integer ) RENAME TO getnodeedges_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function getnodeedges(varchar, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function getnodeedges(varchar, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
$$
LANGUAGE 'plpgsql' VOLATILE;
-- Rename addtopogeometrycolumn ( varchar, varchar, varchar, varchar, varchar, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'addtopogeometrycolumn(varchar, varchar, varchar, varchar, varchar, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function addtopogeometrycolumn(varchar, varchar, varchar, varchar, varchar, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function addtopogeometrycolumn(varchar, varchar, varchar, varchar, varchar, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION addtopogeometrycolumn( varchar, varchar, varchar, varchar, varchar, integer ) RENAME TO addtopogeometrycolumn_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function addtopogeometrycolumn(varchar, varchar, varchar, varchar, varchar, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function addtopogeometrycolumn(varchar, varchar, varchar, varchar, varchar, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
$$
LANGUAGE 'plpgsql' VOLATILE;
-- Rename createtopology ( varchar, integer, float8, boolean ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'createtopology(varchar, integer, float8, boolean)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function createtopology(varchar, integer, float8, boolean) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function createtopology(varchar, integer, float8, boolean) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION createtopology( varchar, integer, float8, boolean ) RENAME TO createtopology_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function createtopology(varchar, integer, float8, boolean) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function createtopology(varchar, integer, float8, boolean) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
$BODY$
LANGUAGE 'plpgsql' VOLATILE STRICT;
-- Alter Type Attribute from integer to bigint -- LastUpdated: 306
SELECT _postgis_topology_upgrade_user_type_attribute('validatetopology_returntype', 'id1', 'integer', 'bigint', '306');

-- Alter Type Attribute from integer to bigint -- LastUpdated: 306
SELECT _postgis_topology_upgrade_user_type_attribute('validatetopology_returntype', 'id2', 'integer', 'bigint', '306');



-- Rename _validatetopologygetfaceshellmaximaledgering ( varchar, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = '_validatetopologygetfaceshellmaximaledgering(varchar, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _validatetopologygetfaceshellmaximaledgering(varchar, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function _validatetopologygetfaceshellmaximaledgering(varchar, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION _validatetopologygetfaceshellmaximaledgering( varchar, integer ) RENAME TO _validatetopologygetfaceshellmaximale_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _validatetopologygetfaceshellmaximaledgering(varchar, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function _validatetopologygetfaceshellmaximaledgering(varchar, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
-- Rename _validatetopologygetringedges ( integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = '_validatetopologygetringedges(integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _validatetopologygetringedges(integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function _validatetopologygetringedges(integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION _validatetopologygetringedges( integer ) RENAME TO _validatetopologygetringedges_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _validatetopologygetringedges(integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function _validatetopologygetringedges(integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
END;
$BODY$ LANGUAGE 'plpgsql';
-- Rename _checkedgelinking ( integer, integer, integer, integer ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = '_checkedgelinking(integer, integer, integer, integer)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _checkedgelinking(integer, integer, integer, integer) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function _checkedgelinking(integer, integer, integer, integer) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION _checkedgelinking( integer, integer, integer, integer ) RENAME TO _checkedgelinking_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _checkedgelinking(integer, integer, integer, integer) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function _checkedgelinking(integer, integer, integer, integer) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
-- Rename validatetopologyrelation ( varchar ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'validatetopologyrelation(varchar)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function validatetopologyrelation(varchar) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function validatetopologyrelation(varchar) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION validatetopologyrelation( varchar ) RENAME TO validatetopologyrelation_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function validatetopologyrelation(varchar) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function validatetopologyrelation(varchar) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
-- Rename removeunusedprimitives ( text, geometry ) deprecated in PostGIS 306, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'removeunusedprimitives(text, geometry)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function removeunusedprimitives(text, geometry) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function removeunusedprimitives(text, geometry) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION removeunusedprimitives( text, geometry ) RENAME TO removeunusedprimitives_deprecated_by_postgis_306;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function removeunusedprimitives(text, geometry) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function removeunusedprimitives(text, geometry) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION topology.postgis_topology_scripts_installed() RETURNS text
	AS $$ SELECT trim('3.6.4'::text || $rev$ 3.6.4 $rev$) AS version $$
	LANGUAGE 'sql' IMMUTABLE;
